...
class Process
{
    ...
    public function getTxnStatus($requestParamList) {
        return $this->callAPI(config('paytm.PAYTM_STATUS_QUERY_URL'), $requestParamList);
    }

    public function getTxnStatusNew($requestParamList) {
        return $this->callNewAPI(config('paytm.PAYTM_STATUS_QUERY_NEW_URL'), $requestParamList);
    }

    public function initiateTxnRefund($requestParamList) {
        $CHECKSUM = $this->getRefundChecksumFromArray($requestParamList, config('paytm.PAYTM_MERCHANT_KEY'),0);
        $requestParamList["CHECKSUM"] = $CHECKSUM;
        return $this->callAPI(config('paytm.PAYTM_REFUND_URL'), $requestParamList);
    }
